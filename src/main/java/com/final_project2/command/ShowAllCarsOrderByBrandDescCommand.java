...
public class ShowAllCarsOrderByBrandDescCommand implements Command {
    private static final Logger log = Logger.getLogger(ShowAllCarsOrderByBrandDescCommand.class);
    ...
    public String execute(HttpServletRequest req, HttpServletResponse resp) throws DBException {
        ...
        List<Car> carsOrderByBrandDesc = CarManager.getInstance().getAllCarsOrderByBrandDesc(pageSize * (page - 1), pageSize);
        req.getSession().setAttribute("allCars", carsOrderByBrandDesc);
        req.getSession().setAttribute("queryVariations", "carsOrderByBrandDesc");
        log.info("all cars order by brand ask " + carsOrderByBrandDesc);
        req.getSession().setAttribute("pageCount", pageCount);
        req.getSession().setAttribute("orderBy", "sort by brand Z-A");
