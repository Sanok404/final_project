...
public class ShowAllCarsOrderByPriceDescCommand implements Command {
    private static final Logger log = Logger.getLogger(ShowAllCarsOrderByPriceDescCommand.class);
    ...
    public String execute(HttpServletRequest req, HttpServletResponse resp) throws DBException {
        ...
        List<Car> carsOrderByPriceDesc = CarManager.getInstance().getAllCarsOrderByPriceDesc(pageSize * (page - 1), pageSize);
        req.getSession().setAttribute("allCars", carsOrderByPriceDesc);
        req.getSession().setAttribute("queryVariations", "carsOrderByPriceDesc");
        log.info("all cars order by Price ask " + carsOrderByPriceDesc.toString());
        req.getSession().setAttribute("pageCount", pageCount);
        req.getSession().setAttribute("orderBy", "sort by decreasing price");
